...
if __name__ == "__main__":
    ...
    def expand_config_recursive(obj, config_keys=("config",), _in_list=False):
        """
        递归展开任意层级中的 'config'（或指定的 config_keys）字典：
        - 若当前是 dict：先递归处理各项；将其中键名在 config_keys 且值为 dict 的部分“提到父层”并合并。
          合并策略：父层已有的显式键优先，不被展开内容覆盖。
        - 若当前是 list/tuple：逐个元素递归处理（不做合并，只下沉处理）。
        - 其它类型：原样返回。

        返回同类型对象；对 dict 返回新 dict；对 list/tuple 返回同类型的新序列。
        """
        # 处理 dict
        if isinstance(obj, Mapping):
            # 先把每个 value 做递归展开
            processed = {k: expand_config_recursive(v, config_keys) for k, v in obj.items()}
            # 收集所有应被“提升”的子 config
            lifted = {}
            rest = {}
            for k, v in processed.items():
                if k in config_keys and isinstance(v, Mapping):
                    # 多个 config 键时，后者覆盖前者（更近的优先）
                    lifted.update(v)
                else:
                    rest[k] = v
            # 合并策略：父层显式键（rest）优先，不被 lifted 覆盖
            return {**lifted, **rest}

        # 处理序列（但不把元素合并，只递归展开其内部）
        if isinstance(obj, Sequence) and not isinstance(obj, (str, bytes, bytearray)):
            t = type(obj)
            return t(expand_config_recursive(x, config_keys, _in_list=True) for x in obj)

        # 其它类型原样返回
        return obj

    configurations = expand_config_recursive(configurations, configurations)
    configurations.pop("follower", None)  # 不存在时不会报错
    configurations.pop("piece_dir", None)  # 不存在时不会报错
